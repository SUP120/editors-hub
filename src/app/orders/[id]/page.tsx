...
export default function OrderDetailsPage({ params }: { params: { id: string } }) {
  ...
  const handlePayment = async () => {
    try {
      ...
      // Send a system message about payment
      await supabase
        .from('messages')
        .insert({
          order_id: orderId,
          ...
          {isClient && order.status === 'accepted' && order.payment_status !== 'completed' && (
            <div className="mb-6">
              <motion.button
                ...
                className="w-full glass-button px-4 py-3 rounded-lg text-sm font-medium text-white flex items-center justify-center gap-2"
              >
